...
'document'(Input, Index) ->
  p(Input, Index, 'document', fun(I,D) -> (p_seq([p_zero_or_more(p_seq([p_optional(fun 'space'/2), fun 'crlf'/2])), p_optional(fun 'doctype'/2), p_optional(fun 'tags'/2), fun 'eof'/2]))(I,D) end, fun(Node, Idx) ->transform('document', Node, Idx) end).

-spec 'doctype'(input(), index()) -> parse_result().
'doctype'(Input, Index) ->
  p(Input, Index, 'doctype', fun(I,D) -> (p_seq([p_optional(fun 'space'/2), p_string(<<"doctype">>), fun 'space'/2, p_label('name', p_one_or_more(p_seq([p_not(fun 'eol'/2), p_anything()]))), fun 'eol'/2]))(I,D) end, fun(Node, Idx) ->transform('doctype', Node, Idx) end).

-spec 'tag'(input(), index()) -> parse_result().
'tag'(Input, Index) ->
  p(Input, Index, 'tag', fun(I,D) -> (p_choose([fun 'comment'/2, fun 'verbatim_text'/2, fun 'tag_item'/2]))(I,D) end, fun(Node, Idx) ->transform('tag', Node, Idx) end).

-spec 'tag_item'(input(), index()) -> parse_result().
'tag_item'(Input, Index) ->
  p(Input, Index, 'tag_item', fun(I,D) -> (p_seq([p_optional(fun 'space'/2), p_choose([fun 'embedded_engine'/2, fun 'inline_html'/2, fun 'code'/2, fun 'slime_tag'/2])]))(I,D) end, fun(Node, Idx) ->transform('tag_item', Node, Idx) end).

-spec 'tags'(input(), index()) -> parse_result().
'tags'(Input, Index) ->
  p(Input, Index, 'tags', fun(I,D) -> (p_one_or_more(p_seq([fun 'tag'/2, p_zero_or_more(fun 'crlf'/2)])))(I,D) end, fun(Node, Idx) ->transform('tags', Node, Idx) end).

-spec 'nested_tags'(input(), index()) -> parse_result().
'nested_tags'(Input, Index) ->
  p(Input, Index, 'nested_tags', fun(I,D) -> (p_seq([p_one_or_more(fun 'crlf'/2), fun 'indent'/2, fun 'tags'/2, fun 'dedent'/2]))(I,D) end, fun(Node, Idx) ->transform('nested_tags', Node, Idx) end).

-spec 'inline_html'(input(), index()) -> parse_result().
'inline_html'(Input, Index) ->
  p(Input, Index, 'inline_html', fun(I,D) -> (p_seq([p_assert(p_string(<<"<">>)), p_one_or_more(fun 'text_item'/2), p_optional(fun 'nested_tags'/2)]))(I,D) end, fun(Node, Idx) ->transform('inline_html', Node, Idx) end).

-spec 'slime_tag'(input(), index()) -> parse_result().
'slime_tag'(Input, Index) ->
  p(Input, Index, 'slime_tag', fun(I,D) -> (p_seq([fun 'tag_shortcut'/2, fun 'tag_spaces'/2, p_optional(fun 'space'/2), fun 'tag_attributes_and_content'/2]))(I,D) end, fun(Node, Idx) ->transform('slime_tag', Node, Idx) end).
...
'tag_content'(Input, Index) ->
  p(Input, Index, 'tag_content', fun(I,D) -> (p_choose([p_string(<<"\/">>), fun 'inline_tag'/2, fun 'nested_tags'/2, fun 'dynamic_content'/2, fun 'inline_text'/2, p_string(<<"">>)]))(I,D) end, fun(Node, Idx) ->transform('tag_content', Node, Idx) end).

-spec 'inline_tag'(input(), index()) -> parse_result().
'inline_tag'(Input, Index) ->
  p(Input, Index, 'inline_tag', fun(I,D) -> (p_seq([p_string(<<":">>), p_optional(fun 'space'/2), fun 'slime_tag'/2]))(I,D) end, fun(Node, Idx) ->transform('inline_tag', Node, Idx) end).

-spec 'inline_text'(input(), index()) -> parse_result().
'inline_text'(Input, Index) ->
  p(Input, Index, 'inline_text', fun(I,D) -> (p_seq([p_not(fun 'eol'/2), fun 'text_block'/2]))(I,D) end, fun(Node, Idx) ->transform('inline_text', Node, Idx) end).
...
'code'(Input, Index) ->
  p(Input, Index, 'code', fun(I,D) -> (p_seq([p_label('output', p_choose([p_seq([p_string(<<"=">>), p_optional(p_string(<<"=">>)), p_optional(fun 'tag_spaces'/2)]), p_string(<<"-">>)])), p_optional(fun 'space'/2), p_label('code', fun 'code_lines'/2), p_label('children', p_optional(fun 'nested_tags'/2)), p_label('optional_else', p_optional(fun 'code_else_condition'/2))]))(I,D) end, fun(Node, Idx) ->transform('code', Node, Idx) end).

-spec 'code_else_condition'(input(), index()) -> parse_result().
'code_else_condition'(Input, Index) ->
  p(Input, Index, 'code_else_condition', fun(I,D) -> (p_seq([p_zero_or_more(fun 'crlf'/2), p_optional(fun 'space'/2), p_string(<<"-">>), p_optional(fun 'space'/2), p_string(<<"else">>), p_label('children', p_optional(fun 'nested_tags'/2))]))(I,D) end, fun(Node, Idx) ->transform('code_else_condition', Node, Idx) end).

-spec 'code_lines'(input(), index()) -> parse_result().
'code_lines'(Input, Index) ->
  p(Input, Index, 'code_lines', fun(I,D) -> (p_choose([fun 'code_line'/2, p_seq([fun 'code_line_with_break'/2, fun 'crlf'/2, fun 'code_lines'/2])]))(I,D) end, fun(Node, Idx) ->transform('code_lines', Node, Idx) end).
...
-spec 'code_line_with_break'(input(), index()) -> parse_result().
'code_line_with_break'(Input, Index) ->
  p(Input, Index, 'code_line_with_break', fun(I,D) -> (p_seq([p_zero_or_more(p_seq([p_not(p_seq([p_anything(), fun 'eol'/2])), p_anything()])), fun 'code_line_break'/2, p_assert(fun 'eol'/2)]))(I,D) end, fun(Node, Idx) ->transform('code_line_with_break', Node, Idx) end).
...
'string_with_interpolation'(Input, Index) ->
  p(Input, Index, 'string_with_interpolation', fun(I,D) -> (p_seq([p_string(<<"\"">>), p_zero_or_more(p_choose([fun 'elixir_interpolation'/2, p_seq([p_string(<<"\\">>), p_anything()]), p_seq([p_not(p_string(<<"\"">>)), p_anything()])])), p_string(<<"\"">>)]))(I,D) end, fun(Node, Idx) ->transform('string_with_interpolation', Node, Idx) end).
...
-spec 'text_item'(input(), index()) -> parse_result().
'text_item'(Input, Index) ->
  p(Input, Index, 'text_item', fun(I,D) -> (p_choose([p_label('static', fun 'text'/2), p_label('dynamic', p_choose([p_label('safe', fun 'safe_interpolation'/2), fun 'interpolation'/2]))]))(I,D) end, fun(Node, Idx) ->transform('text_item', Node, Idx) end).

-spec 'text'(input(), index()) -> parse_result().
'text'(Input, Index) ->
  p(Input, Index, 'text', fun(I,D) -> (p_one_or_more(p_choose([p_seq([p_string(<<"\\">>), p_anything()]), p_seq([p_not(p_choose([p_string(<<"#{">>), fun 'eol'/2])), p_anything()])])))(I,D) end, fun(Node, Idx) ->transform('text', Node, Idx) end).

-spec 'elixir_interpolation'(input(), index()) -> parse_result().
'elixir_interpolation'(Input, Index) ->
  p(Input, Index, 'elixir_interpolation', fun(I,D) -> (p_seq([p_string(<<"#{">>), p_zero_or_more(p_choose([fun 'string'/2, fun 'string_with_interpolation'/2, p_seq([p_not(p_string(<<"}">>)), p_anything()])])), p_string(<<"}">>)]))(I,D) end, fun(Node, Idx) ->transform('elixir_interpolation', Node, Idx) end).
...
-spec 'safe_interpolation'(input(), index()) -> parse_result().
'safe_interpolation'(Input, Index) ->
  p(Input, Index, 'safe_interpolation', fun(I,D) -> (p_seq([p_string(<<"#{{">>), p_zero_or_more(p_choose([fun 'string'/2, fun 'string_with_interpolation'/2, p_seq([p_not(p_string(<<"}}">>)), p_anything()])])), p_string(<<"}}">>)]))(I,D) end, fun(Node, Idx) ->transform('safe_interpolation', Node, Idx) end).
...
'comment'(Input, Index) ->
  p(Input, Index, 'comment', fun(I,D) -> (p_choose([fun 'html_comment'/2, p_seq([p_optional(fun 'space'/2), fun 'code_comment'/2])]))(I,D) end, fun(Node, Idx) ->transform('comment', Node, Idx) end).

-spec 'html_comment'(input(), index()) -> parse_result().
'html_comment'(Input, Index) ->
  p(Input, Index, 'html_comment', fun(I,D) -> (p_seq([p_label('indent', p_optional(fun 'space'/2)), p_label('type', p_string(<<"\/!">>)), p_label('content', fun 'text_block'/2)]))(I,D) end, fun(Node, Idx) ->transform('html_comment', Node, Idx) end).

-spec 'code_comment'(input(), index()) -> parse_result().
'code_comment'(Input, Index) ->
  p(Input, Index, 'code_comment', fun(I,D) -> (p_seq([p_string(<<"\/">>), fun 'text_block'/2]))(I,D) end, fun(Node, Idx) ->transform('code_comment', Node, Idx) end).

-spec 'verbatim_text'(input(), index()) -> parse_result().
'verbatim_text'(Input, Index) ->
  p(Input, Index, 'verbatim_text', fun(I,D) -> (p_seq([p_label('indent', p_optional(fun 'space'/2)), p_label('type', p_charclass(<<"[|\']">>)), p_label('content', fun 'text_block'/2)]))(I,D) end, fun(Node, Idx) ->transform('verbatim_text', Node, Idx) end).

-spec 'text_block'(input(), index()) -> parse_result().
'text_block'(Input, Index) ->
  p(Input, Index, 'text_block', fun(I,D) -> (p_seq([fun 'indented_text_line'/2, p_optional(p_seq([fun 'crlf'/2, fun 'indent'/2, p_label('lines', fun 'text_block_nested_lines'/2), fun 'dedent'/2]))]))(I,D) end, fun(Node, Idx) ->transform('text_block', Node, Idx) end).

-spec 'text_block_nested_lines'(input(), index()) -> parse_result().
'text_block_nested_lines'(Input, Index) ->
  p(Input, Index, 'text_block_nested_lines', fun(I,D) -> (p_seq([fun 'indented_text_line'/2, p_zero_or_more(p_seq([fun 'crlf'/2, p_choose([p_seq([fun 'indent'/2, p_label('lines', fun 'text_block_nested_lines'/2), fun 'dedent'/2]), p_label('lines', fun 'text_block_nested_lines'/2)])]))]))(I,D) end, fun(Node, Idx) ->transform('text_block_nested_lines', Node, Idx) end).

-spec 'embedded_engine'(input(), index()) -> parse_result().
'embedded_engine'(Input, Index) ->
  p(Input, Index, 'embedded_engine', fun(I,D) -> (p_seq([fun 'tag_name'/2, p_string(<<":">>), p_seq([fun 'crlf'/2, fun 'indent'/2, p_label('lines', fun 'embedded_engine_lines'/2), fun 'dedent'/2])]))(I,D) end, fun(Node, Idx) ->transform('embedded_engine', Node, Idx) end).

-spec 'embedded_engine_lines'(input(), index()) -> parse_result().
'embedded_engine_lines'(Input, Index) ->
  p(Input, Index, 'embedded_engine_lines', fun(I,D) -> (p_seq([fun 'indented_text_line'/2, p_zero_or_more(p_seq([fun 'crlf'/2, fun 'indented_text_line'/2]))]))(I,D) end, fun(Node, Idx) ->transform('embedded_engine_lines', Node, Idx) end).

-spec 'indented_text_line'(input(), index()) -> parse_result().
'indented_text_line'(Input, Index) ->
  p(Input, Index, 'indented_text_line', fun(I,D) -> (p_seq([p_optional(fun 'space'/2), p_zero_or_more(fun 'text_item'/2)]))(I,D) end, fun(Node, Idx) ->transform('indented_text_line', Node, Idx) end).
